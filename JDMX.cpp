...
ISR (USART_TX)
{
    ...
    case isr::RDMTransmitComplete:
           // Start waitin for new data
        if (__rdm_controller == NULL) //If we're just a responder device
        {
            SetISRMode ( isr::Receive ); 
...
//RDM Controller Class

//
// master parameter is only used to ensure a slave object is present before
// initializing the rdm responder class
//
RDM_Controller::RDM_Controller ( uint16_t m, uint8_t d1, uint8_t d2, 
                               uint8_t d3, uint8_t d4, DMX_Master &master)
:   RDM_FrameBuffer ( )
{
