/*
  JDMX.h - DMX library for Arduino
...
class DMX_Master
{
    public:
        ...
        DMX_Master (DMX_FrameBuffer &buffer, int readEnablePin , int transmitEnablePin, bool inverted );
        
        // Run the DMX master by giving a predefined maximum number of
        // channels to support
        DMX_Master (uint16_t maxChannel, int readEnablePin , int transmitEnablePin, bool inverted );
...
class RDM_Controller : public RDM_FrameBuffer
{
    public:
        ...
        void RDM_Controller::send_DISC_UNIQUE_BRANCH(uint16_t dest_m_id, uint8_t ddid1,  uint8_t ddid2, uint8_t ddid3, uint8_t ddid4, RDM_Uid lowerBound, RDM_Uid upperBound, uint8_t TN = 0, uint8_t port = 1, uint8_t sub = 0);
